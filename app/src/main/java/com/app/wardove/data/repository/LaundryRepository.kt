...
class LaundryRepository @Inject constructor(
    ...
    /**
     * Creates the cycle, links its items and flips their status in one transaction —
     * a failure part-way through must not leave items stuck IN_LAUNDRY with no cycle
     * (or a cycle with no items).
     */
